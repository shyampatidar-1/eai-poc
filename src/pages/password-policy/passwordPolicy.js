...
import { addPasswordPolicy, getPasswordPolicy } from "../../hooks/services/api-services";
...
const PasswordPolicy = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await addPasswordPolicy(payload);
  ...
  const passwordPolicyList = async () => {
    ...
    try {
      ...
      if (response.data?.status === 200) {
        ...
        // setPolicies(response.data?.data?.data?.content)
      }
    } catch (error) {
      toastEmitter("error", API_RESPONSE?.MESSAGE_503);
    }
